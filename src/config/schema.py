...
class RuntimeConfig(StrictModel):
    index_visibility_sla: str | None = None
    draft_media_ttl: str = Field(min_length=1)


class VisualPreprocessingConfig(StrictModel):
    image_size: tuple[int, int]
    interpolation: str
    normalization: str


class VisualExpectationsConfig(StrictModel):
    expected_device: str = Field(min_length=1)
    model_name: str = Field(min_length=1)
    model_version: str = Field(min_length=1)
    preprocessing: VisualPreprocessingConfig
...
class AppConfig(StrictModel):
    ...
    visual_expectations: VisualExpectationsConfig
    llm: LlmContract
